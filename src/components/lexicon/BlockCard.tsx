...
export function BlockCard({ block, index, active, onActivate, onChange, onDelete }: BlockCardProps) {
  ...
  return (
    <Reorder.Item
      ...
      className="list-none"
    >
      <DragHandleWrapper blockId={block.id}>
        {(controls, dragging) => (
          <motion.div
            layout
          ...
          >
            <div
              ...
              style={
                active
                  ? {
                      boxShadow: `inset 0 0 0 1px hsl(${meta.hsl} / 0.6), 0 0 32px hsl(${meta.hsl} / 0.28)`,
                    }
                  : undefined
              }
            >
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(180deg, hsl(${meta.hsl} / 0.9), hsl(${meta.hsl} / 0.2))`,
              ...
              <div className="relative flex items-start gap-3">
                ...
                <button
                  ...
                  className={`mt-0.5 flex h-8 w-8 shrink-0 cursor-grab items-center justify-center rounded-md border border-border/60 bg-background/40 text-muted-foreground transition-all hover:text-foreground active:cursor-grabbing ${
                    dragging ? "bg-background text-foreground" : ""
                  }`}
                ...
                <AnimatePresence mode="wait" initial={false}>
                  {editing ? (
                    <motion.textarea
                      ...
                      style={{
                        boxShadow: `inset 0 0 0 1px hsl(${meta.hsl} / 0.5)`,
                      }}
                    />
                  ) : (
                    <motion.p
...
function DragHandleWrapper({
  ...
  blockId: string;
  children: (controls: ReturnType<typeof useDragControls>, dragging: boolean) => React.ReactNode;
}) {
  const controls = useDragControls();
  // Reorder.Item must receive dragControls via context; we re-attach via prop on parent
  return <InnerItem controls={controls}>{children}</InnerItem>;
}
...
function InnerItem({
  ...
  // We rely on Reorder.Item's own gesture; we just expose controls so the handle can call .start
  // Actual binding happens via dragControls prop on Reorder.Item — set in parent via `data-`?
  // framer-motion supports passing dragControls to Reorder.Item via prop. We do that in BlockCard above.
  return <>{children(controls, false)}</>;
}
